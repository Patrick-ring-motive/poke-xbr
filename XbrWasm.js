(() => {
  ...
  self.XbrWasm = class XbrWasm {
    constructor(t, s, e) {
      if (e || (e = {}), e.trY || (e.trY = 48 / 255), e.trU || (e.trU = 48 / 255), e.trV || (e.trV = 48 / 255), e.algorithm || (e.algorithm = "xbr"), this.options = e, this.srcElement = t, this.srcElementType = t.nodeName.toLowerCase(), this.srcElementLastTime = -1, "canvas" === this.srcElementType) this.srcCanvas = t, this.srcCanvasCtx = this.srcCanvas.getContext("2d");
      else {
        if ("img" !== this.srcElementType && "video" !== this.srcElementType) throw "XbrWasm: Unsupported Element Type. Valid types: 'canvas', 'img', 'video'";
        {
          this.srcCanvas = document.createElement("canvas"), this.srcCanvasCtx = this.srcCanvas.getContext("2d");
          const s = "img" === this.srcElementType ? this.srcElement.naturalWidth : this.srcElement.videoWidth,
            e = "img" === this.srcElementType ? this.srcElement.naturalHeight : this.srcElement.videoHeight;
          this.srcCanvas.width = s, this.srcCanvas.height = e, this.srcCanvasCtx.drawImage(t, 0, 0)
        }
      }
      this.instanceId = XbrWasm.instanceCount, XbrWasm.instanceCount += 1, this.scalingFactor = s, this.destCanvas = document.createElement("canvas"), this.destCanvas.width = this.srcCanvas.width * s, this.destCanvas.height = this.srcCanvas.height * s, this.destCanvasCtx = this.destCanvas.getContext("2d");
      const a = this.srcCanvas.width * this.srcCanvas.height * 4 * this.scalingFactor * this.scalingFactor;
      this.destDataBuffer = new ArrayBuffer(a), this.jobRunning = !1, this.debugFrameCount = 1, this.trY = e.trY, this.trU = e.trU, this.trV = e.trV
    }
    async draw() {
      if (this.jobRunning) return 1;
      "video" === this.srcElementType && this.srcCanvasCtx.drawImage(this.srcElement, 0, 0), this.jobRunning = !0;
      const t = this.srcCanvasCtx.getImageData(0, 0, this.srcCanvas.width, this.srcCanvas.height).data.buffer;
      this.destDataBuffer = await this.scale(t, this.destDataBuffer);
      const s = new Uint8ClampedArray(this.destDataBuffer);
      this.destCanvas.getContext("2d").putImageData(new ImageData(s, this.destCanvas.width, this.destCanvas.height), 0, 0), this.jobRunning = !1
    }
    async drawContinuous() {
      if ("video" !== this.srcElementType)
        for (;;) await this.draw();
      else {
        for (; this.srcElement.currentTime !== this.srcElementLastTime;) await this.draw(), this.srcElementLastTime = this.srcElement.currentTime;
        requestAnimationFrame(this.drawContinuous.bind(this))
      }
    }
    async benchmark(t) {
      const s = performance.now();
      for (let s = 0; s < t - 1; s++) await this.draw(), this.debugDraw("Frame: " + this.debugFrameCount), this.debugFrameCount++;
      await this.draw();
      const e = performance.now(),
        a = t / (e - s) * 1e3;
      this.debugDraw("Benchmark: " + t + " frames in " + ((e - s) / 1e3).toFixed(2) + "s. " + a.toFixed(0) + "fps average."), this.debugFrameCount = 0
    }
    debugDraw(t) {
      this.destCanvasCtx.font = "bold 12px sans-serif", this.destCanvasCtx.fillStyle = "black";
      const s = this.destCanvasCtx.measureText(t);
      this.destCanvasCtx.fillRect(5, 5, s.width + 10, 22), this.destCanvasCtx.fillStyle = "white", this.destCanvasCtx.fillText(t, 10, 22)
    }
    scale(t, s) {
      let e = this;
      return new Promise((a, r) => {
        XbrWasm.worker.addEventListener("message", function message(s) {
          "complete" === s.data.status && s.data.instanceId === e.instanceId && (a(s.data.destData), XbrWasm.worker.removeEventListener("message", message))
        }), XbrWasm.worker.postMessage({
          instanceId: e.instanceId,
          srcData: t,
          destData: s,
          scalingFactor: e.scalingFactor,
          srcCanvasWidth: e.srcCanvas.width,
          srcCanvasHeight: e.srcCanvas.height,
          options: this.options
        }, [t, s])
      })
    }
  }
  XbrWasm.instanceCount = 0;
  const workerURL = document.currentScript.src;
  XbrWasm.worker = new Worker(workerURL), XbrWasm.ready = new Promise((t, s) => {
    XbrWasm.worker.addEventListener("message", function message(e) {
      "ready" === e.data.status && (t("ready"), XbrWasm.worker.removeEventListener("message", message))
    })
  }), XbrWasm.vibecheck = function() {
    var t;
    console.log((t = (t => "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz".indexOf(t)), "Ivor purpx! Lbh'er hfvat gur cebcevrgnel 'koe-udk-jnfz' yvoenel ol Znkvzvyyvna Ynhzrvfgre. Guvf yvoenel vf serr sbe ABAPBZZREPVNY HFR BAYL. Sbe abapbzzrepvny yvprafr qrgnvyf, naq gb chepunfr n pbzzrepvny yvprafr, cyrnfr frr uggcf://jjj.znkynhzrvfgre.pbz/koe-udk-jnfz/".split("").map(s => t(s) > -1 ? "NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm" [t(s)] : s).join("")))
  };
...
(() => {
  ...
  self.onmessage = function(e) {
    const n = e.data.instanceId,
      r = e.data.destData.byteLength,
      t = e.data.options;
    Math.floor(255 * t.trY), Math.floor(255 * t.trU), Math.floor(255 * t.trV);
    let a, o;
    nativeBuffers[n] && nativeBuffers[n].byteLength === r ? a = nativeBuffers[n].buffer : (a = Module._malloc(r), nativeBuffers[n] && Module._free(nativeBuffers[n].buffer), nativeBuffers[n] = {
      byteLength: r,
      buffer: a
    });
    try {
      if ("xbr" !== t.algorithm) throw "XbrWasm: Unsupported scaling algorithm";
      Module.ccall("scaleImageXbr", null, ["array", "number", "number", "number", "number", "number", "number"], [new Uint8ClampedArray(e.data.srcData), e.data.srcData.byteLength, a, r, e.data.scalingFactor, e.data.srcCanvasWidth, e.data.srcCanvasHeight])
    } catch (e) {
      o = e
    }
    if (o) throw o;
    const i = Module.HEAPU8.subarray(a, a + r),
      u = new Uint8ClampedArray(e.data.destData);
    u.set(i), self.postMessage({
      status: "complete",
      instanceId: n,
      destData: u.buffer
    }), u.buffer
  };
  var key, Module = void 0 !== (Module = {
      preRun: [],
      postRun: [function() {
        self.postMessage({
          status: "ready"
  ...
  ENVIRONMENT_IS_NODE ? (scriptDirectory = __dirname + "/", read_ = function(e, n) {
    return nodeFS || (nodeFS = require("fs")), nodePath || (nodePath = require("path")), e = nodePath.normalize(e), nodeFS.readFileSync(e, n ? null : "utf8")
    ...
    var n;
    return "function" == typeof readbuffer ? new Uint8Array(readbuffer(e)) : (assert("object" == typeof(n = read(e, "binary"))), n)
  ...
  var wasmBinary, noExitRuntime, wasmMemory, out = Module.print || console.log.bind(console),
  ...
  moduleOverrides = null, Module.arguments && (arguments_ = Module.arguments), Module.thisProgram && (thisProgram = Module.thisProgram), Module.quit && (quit_ = Module.quit), Module.wasmBinary && (wasmBinary = Module.wasmBinary), Module.noExitRuntime && (noExitRuntime = Module.noExitRuntime), "object" != typeof WebAssembly && err("no native wasm support detected");
  ...
  function getCFunc(e) {
    var n = Module["_" + e];
    return assert(n, "Cannot call unknown function " + e + ", make sure it is exported"), n
  }

  function ccall(e, n, r, t, a) {
    var o = {
      string: function(e) {
        var n = 0;
        if (null != e && 0 !== e) {
          var r = 1 + (e.length << 2);
          stringToUTF8(e, n = stackAlloc(r), r)
        }
        return n
      },
      array: function(e) {
    ...
    if (t)
      for (var s = 0; s < t.length; s++) {
        var c = o[r[s]];
        c ? (0 === l && (l = stackSave()), u[s] = c(t[s])) : u[s] = t[s]
      }
  ...
  function UTF8ArrayToString(e, n, r) {
    ...
    if (a - n > 16 && e.subarray && UTF8Decoder) return UTF8Decoder.decode(e.subarray(n, a));
    for (var o = ""; n < a;) {
      var i = e[n++];
      if (128 & i) {
        var u = 63 & e[n++];
        if (192 != (224 & i)) {
          var l = 63 & e[n++];
          if ((i = 224 == (240 & i) ? (15 & i) << 12 | u << 6 | l : (7 & i) << 18 | u << 12 | l << 6 | 63 & e[n++]) < 65536) o += String.fromCharCode(i);
          else {
            var s = i - 65536;
  ...
  function stringToUTF8Array(e, n, r, t) {
    if (!(t > 0)) return 0;
    for (var a = r, o = r + t - 1, i = 0; i < e.length; ++i) {
      ...
      if (u <= 127) {
        ...
        if (r + 1 >= o) break;
        n[r++] = 192 | u >> 6, n[r++] = 128 | 63 & u
      } else if (u <= 65535) {
        if (r + 2 >= o) break;
        n[r++] = 224 | u >> 12, n[r++] = 128 | u >> 6 & 63, n[r++] = 128 | 63 & u
      } else {
        if (r + 3 >= o) break;
        n[r++] = 240 | u >> 18, n[r++] = 128 | u >> 12 & 63, n[r++] = 128 | u >> 6 & 63, n[r++] = 128 | 63 & u
      }
    }
    return n[r] = 0, r - a
  }

  function stringToUTF8(e, n, r) {
    return stringToUTF8Array(e, HEAPU8, n, r)
  }

  function lengthBytesUTF8(e) {
    for (var n = 0, r = 0; r < e.length; ++r) {
      var t = e.charCodeAt(r);
      t >= 55296 && t <= 57343 && (t = 65536 + ((1023 & t) << 10) | 1023 & e.charCodeAt(++r)), t <= 127 ? ++n : n += t <= 2047 ? 2 : t <= 65535 ? 3 : 4
    }
    return n
  }
  ...
  function allocateUTF8OnStack(e) {
    var n = lengthBytesUTF8(e) + 1,
      r = stackAlloc(n);
    return stringToUTF8Array(e, HEAP8, r, n), r
  }
  ...
  var DYNAMIC_BASE = 72353440,
    DYNAMICTOP_PTR = 67110400,
    INITIAL_TOTAL_MEMORY = Module.TOTAL_MEMORY || 134217728;
  ...
  function callRuntimeCallbacks(e) {
    for (; e.length > 0;) {
      ...
      if ("function" != typeof n) {
        var r = n.func;
        "number" == typeof r ? void 0 === n.arg ? Module.dynCall_v(r) : Module.dynCall_vi(r, n.arg) : r(void 0 === n.arg ? null : n.arg)
      } else n()
    }
  ...
  var wasmBinaryFile = "https://poke-scrape.patrickring.net/XbrWasmPayload.wasm";
  ...
  function getBinaryPromise() {
    return wasmBinary || !ENVIRONMENT_IS_WEB && !ENVIRONMENT_IS_WORKER || "function" != typeof fetch ? new Promise(function(e, n) {
      e(getBinary())
  ...
  function createWasm() {
    ...
    return function() {
      if (wasmBinary || "function" != typeof WebAssembly.instantiateStreaming || isDataURI(wasmBinaryFile) || "function" != typeof fetch) return t(r);
      fetch(wasmBinaryFile, {
        credentials: "same-origin"
      }).then(function(n) {
        return WebAssembly.instantiateStreaming(n, e).then(r, function(e) {
          err("wasm streaming compile failed: " + e), err("falling back to ArrayBuffer instantiation"), t(r)
        })
      })
    }(), {}
  }
  ...
  var calledRun, ___wasm_call_ctors = Module.___wasm_call_ctors = function() {
    ...
    _scaleImageXbr = Module._scaleImageXbr = function() {
  ...
  function callMain(e) {
    ...
    for (var a = 1; a < r; a++) HEAP32[(t >> 2) + a] = allocateUTF8OnStack(e[a - 1]);
    HEAP32[(t >> 2) + r] = 0;
    try {
      exit(n(r, t), !0)
    } catch (e) {
      ...
      var o = e;
      e && "object" == typeof e && e.stack && (o = [e, e.stack]), err("exception thrown: " + o), quit_(1, e)
    } finally {
      calledMain = !0
    }
  }
  ...
  function run(e) {
    function n() {
      calledRun || (calledRun = !0, ABORT || (initRuntime(), preMain(), Module.onRuntimeInitialized && Module.onRuntimeInitialized(), shouldRunNow && callMain(e), postRun()))
    }
  ...
  function exit(e, n) {
    n && noExitRuntime && 0 === e || (noExitRuntime || (ABORT = !0, EXITSTATUS = e, exitRuntime(), Module.onExit && Module.onExit(e)), quit_(e, new ExitStatus(e)))
  }
